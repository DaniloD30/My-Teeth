...
const ClinicTypeForm = (props) => {
  ...
  // );
  const addClinicTypeLoading = useSelector(
    (state) => state.app?.loading?.addClinicTypeLoading
  );

  const editClinicTypeLoading = useSelector(
    (state) => state.app?.loading?.editClinicTypeLoading
  );

  let clinic = {
    name: "Clinica dasdadd",
    description: "Sim",
    active: true
  };
          ...
          <Formik
            ...
            onSubmit={(values, { setSubmitting }) => {
              if (location?.state) {
                if (isAuthenticated()) {
                  dispatch(
                    clinicTypeAction.editClinicType(
                      values,
                      getToken(),
                      location?.state?.id,
                      "editClinicTypeLoading",
                      (error) => {
                        setSubmitting(false);
                        ...
                        Utils.showToast({
                          type: "success",
                          description: "Tipo de Clínica editada com sucesso!",
                        });
                ...
                if (isAuthenticated()) {
                  dispatch(
                    clinicTypeAction.addClinicType(
                      values,
                      getToken(),

                      "addClinicTypeLoading",
                      (error) => {
                        setSubmitting(false);
                        ...
                        Utils.showToast({
                          type: "success",
                          description: "Tipo de Clínica cadastrada com sucesso!",
                        });
                    ...
                    <Grid item xs={2}>
                      <Box className="button-form">
                        <Button
                          ...
                          disableElevation
                          disabled={
                            addClinicTypeLoading || editClinicTypeLoading
                          }
                          onClick={submitForm}
                          // disabled={loading.driver}
...
export default withRouter(ClinicTypeForm);
